...
export function KPICards({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card className="border-t-4" style={{ borderTopColor: "var(--chart-1)" }}>
        ...
        <CardContent>
          ...
          <p className={`text-xs ${revenueChange >= 0 ? "text-green-600" : "text-red-600"} font-bold`}>
            {revenueChange >= 0 ? "+" : ""}{revenueChange.toFixed(1)}% from last period
          </p>
        </CardContent>
      </Card>

      <Card className="border-t-4" style={{ borderTopColor: "var(--chart-2)" }}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <CardContent>
          ...
          <p className={`text-xs ${ordersChange >= 0 ? "text-green-600" : "text-red-600"} font-bold`}>
            {ordersChange >= 0 ? "+" : ""}{ordersChange.toFixed(1)}% from last period
          </p>
        </CardContent>
      </Card>

      <Card className="border-t-4" style={{ borderTopColor: "var(--chart-3)" }}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card className="border-t-4" style={{ borderTopColor: "var(--chart-4)" }}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
